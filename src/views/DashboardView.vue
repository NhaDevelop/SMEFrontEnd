...
import { getReadinessStatus } from '@/utils/mockData'
...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      await assessmentStore.fetchAssessmentData()
      await actionsStore.fetchActions()
    })

    const companyInfo = computed(() => userStore.companyInfo)
    const pillars = computed(() => assessmentStore.pillars)
    const progressData = computed(() => assessmentStore.progressData)
    const overallScore = computed(() => assessmentStore.overallScore)
    const growthPotential = computed(() => assessmentStore.growthPotential)
    const overallRiskLevel = computed(() => assessmentStore.overallRiskLevel)
    const scoreChange = computed(() => assessmentStore.scoreChange)
    const actionsCount = computed(() => actionsStore.actionsCount)
    const recommendedActions = computed(() => actionsStore.actions)
