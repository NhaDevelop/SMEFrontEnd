<template>
  <div class="overflow-x-auto">
    ...
    <div v-if="weakPillars.length > 0" class="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-lg">
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1">
          ...
          <p class="text-xs text-amber-700 mb-2">
            {{ weakPillars.length }} pillar{{ weakPillars.length > 1 ? 's' : '' }} {{ weakPillars.length > 1 ? 'are' : 'is' }} underperforming and need improvement:
          </p>
          <div class="flex flex-wrap gap-2">
            <span v-for="pillar in weakPillars" :key="pillar.id" 
...
export default {
  ...
  methods: {
    ...
    weakPillars() {
      ...
      return this.pillars.filter(p => (p.score || 0) < threshold)
    },
    goToWhatIf(pillarId) {
      navigateTo({
        path: '/sme/what-if',
