  ...
  <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm mt-6">
    ...
    <div class="mb-8">
        ...
        <div class="flex gap-4 mt-3 text-xs text-gray-500">
            <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-red-500"></span> {{ highPriorityCount }} High</span>
            <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-amber-500"></span> {{ mediumPriorityCount }} Medium</span>
        ...
        <div v-for="(action, idx) in actions" :key="idx" 
            ...
            <div class="flex-1">
                <div class="flex items-center gap-2 mb-1">
                    <span :class="['px-2 py-0.5 rounded text-[10px] uppercase font-bold border', getPriorityBadge(action.priority)]">
                        {{ action.priority }}
                    </span>
                ...
                <h4 :class="['text-sm font-bold transition-colors', action.completed ? 'text-gray-500 line-through' : 'text-gray-900']">{{ action.title }}</h4>
                <p v-if="!action.completed" class="text-xs text-[#198754] mt-1 font-bold">Expected improvement: +{{ action.points }} points</p>
