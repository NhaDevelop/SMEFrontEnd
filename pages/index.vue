  ...
  <div class="min-h-screen flex">
    ...
    <div class="hidden lg:flex lg:w-1/2 bg-navy-900 text-white p-12 flex-col justify-between">
      <div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div v-for="pillar in pillars" :key="pillar" class="flex items-center gap-2 text-sm">
    ...
    <div class="flex-1 flex items-center justify-center p-8 bg-gray-50">
      <div class="w-full max-w-md">
        ...
        <!-- Role Cards -->
        <div class="space-y-4">
          <RoleCard role="sme_user" title="SME User"
            description="Access your investment readiness dashboard and improvement actions" icon-color="cyan"
            @select="handleRoleSelect" />

          <RoleCard role="investor" title="Investor / Program"
            description="View portfolio overview, compare SMEs, and analyze opportunities" icon-color="green"
            @select="handleRoleSelect" />

          <RoleCard role="administrator" title="Administrator"
            description="Manage users, programs, and assessment framework settings" icon-color="purple"
            @select="handleRoleSelect" />
        </div>
      </div>
    </div>
...
export default {
  ...
  setup() {
    ...
    const handleRoleSelect = async (role) => {
      ...
      const mappedRole = role === 'sme_user' ? 'SME' : role === 'investor' ? 'INVESTOR' : 'ADMIN'

      // Perform login using the store
      await authStore.login({ role: mappedRole })

      // Navigate based on role
      if (mappedRole === 'INVESTOR') {
